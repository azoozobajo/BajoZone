...
try {
    ...
    foreach (($data['programs'] ?? []) as $program) {
        ...
        $upsertProgram->execute([
            $id, $title, $slug, $description, $content, $image, $shortDescription,
    ...
    foreach (($data['articles'] ?? []) as $article) {
        ...
        $programId = isset($article['program_id']) && $article['program_id'] !== '' ? (string) $article['program_id'] : null;
        ...
        $upsertArticle->execute([
            $id, $title, $slug, $excerpt, $content, $image, $categoryId, $programId,
    ...
    foreach (($data['books'] ?? []) as $book) {
        ...
        $slug = importSlug((string) ($book['slug'] ?? $book['title_en'] ?? $title), $id);
        $upsertBook->execute([
            $id, $title, $slug,
    ...
    foreach (($data['resources'] ?? []) as $resource) {
        ...
        $slug = importSlug((string) ($resource['slug'] ?? $resource['title_en'] ?? $title), $id);
        ...
        $upsertResource->execute([
            ...
            importDateOrNull($resource['created_at'] ?? null),
            importDateOrNull($resource['updated_at'] ?? null),
        ]);
    }
