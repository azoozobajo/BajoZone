...
function renderAboutPremium() {
  ...
  const filmSlotsHtml = filmItems.length
    ? filmItems.map((item, idx) => `
      <figure class="about-film-slot" data-film-slot="${idx}">
        <img src="${aboutEsc(item.src)}" alt="${aboutEsc(item.title || (isAr ? 'محطة من الرحلة' : 'Journey moment'))}" loading="${idx < 4 ? 'eager' : 'lazy'}" decoding="async">
  ...
  const logoTrack = logoItems || AboutPremium.logos.map((logo, idx) => {
    const rawLabel = (isAr ? logo.alt_ar : logo.alt_en) || logo.alt_ar || logo.alt_en || `Logo ${idx + 1}`;
    const label = aboutEsc(rawLabel);
    const abbr = aboutEsc(logo.abbr || String(idx + 1));
    return `<div class="about-logo-item" data-logo-label="${label}">
      <div class="about-logo-card">
        <span class="about-logo-abbr">${abbr}</span>
        <span class="about-logo-name">${label}</span>
  ...
  requestAnimationFrame(() => {
    ...
    /* Photos are rendered directly so every uploaded image is visible. */
  });
}
